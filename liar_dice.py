            ...
            if i == self.max_actions:
                ...
                if current_state == -1:
                    if (player, current_state) in plays and len(plays[(player, current_state)]) == 7:
                        ...
                        log_total = np.log(
                            np.sum(plays[(player, current_state)][strategy] for strategy in [1, 2, 3, 4, 5, 6, 7]))
                        value, strategy = max(
                            ((wins[(player, current_state)][strategy] / plays[(player, current_state)][strategy]) +
                             self.confident * np.sqrt(2 * log_total / plays[(player, current_state)][strategy]),
                             strategy)
                    ...
                    if (player, current_state) in plays and len(plays[(player, current_state)]) == 7:
                        ...
                        log_total = np.log(
                            np.sum(plays[(player, current_state)][strategy] for strategy in self.strategy_pool()))
                        value, strategy = max(
                            ((wins[(player, current_state)][strategy] / plays[(player, current_state)][strategy]) +
                             self.confident * np.sqrt(2 * log_total / plays[(player, current_state)][strategy]),
                             strategy)
